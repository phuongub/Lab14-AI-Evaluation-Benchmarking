...
async def run_benchmark_with_results(agent_instance, agent_version: str, judge_engine: LLMJudge):
    ...
    for r in results:
        ...
        for _, judge_result in individual.items():
            criteria = judge_result.get("criteria", {})
            if "faithfulness" in criteria and criteria["faithfulness"] is not None:
                case_scores.append(criteria["faithfulness"])

        if case_scores:
            faithfulness_scores.append(sum(case_scores) / len(case_scores))
